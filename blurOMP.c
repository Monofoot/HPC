...
int main (int argc, const char * argv[]) {
  ...
  // Must remove the const designation as OpenMP predetermines them to be shared.
  static int maxlen = 200, rowsize = 521, colsize = 428, linelen = 12;
  ...
  // This becomes our iterations, N.
	nblurs = 10;
	gettimeofday(&tim, NULL);
	double t1=tim.tv_sec+(tim.tv_usec/1000000.0);

  // Set chunk to equal CHUNKSIZE.
  // Also initialize int i for iterating.
  int chunk;
  chunk = CHUNKSIZE;

  // Begin parallelization.
  #pragma omp parallel shared(nblurs, rowsize, Rnew, chunk) num_threads(8)
  {
    // Now schedule a static or dynamic loop. Simply swap dynamic for static.
